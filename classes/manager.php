...
class manager {
    ...
    public static function export_configuration(array $sections = []): array {
        ...
        if (in_array(self::EXPORT_SECTION_SLIDESHOW, $sections, true)) {
            $export['sections'][self::EXPORT_SECTION_SLIDESHOW] = self::export_easyedu_slideshow_configuration();
        }
        if (in_array(self::EXPORT_SECTION_SITE_BANNERS, $sections, true)) {
            $export['sections'][self::EXPORT_SECTION_SITE_BANNERS] = self::export_easyedu_site_banner_configuration();
        }
    ...
    protected static function export_course_banner_configuration(): array {
        ...
        foreach (array_keys($sourcekeys) as $sourcekey) {
            ...
            $sourceexport = self::export_source_configuration($sourcekey, $source, $settings, $elements);
            if (!$sourceexport) {
                continue;
            }
    ...
    protected static function export_source_configuration(
        ...
        foreach ($elementrecords as $record) {
            ...
            $elements[] = self::export_element_record($record);
        }
        ...
        if ($sourcetype === self::SOURCE_TYPE_CATEGORY) {
     ...
     * @return array
     */
    protected static function export_element_record(\stdClass $record): array {
        $file = self::get_banner_image_file($record);
        return [
            ...
            'archivepath' => $file ? 'coursebanners/elements/' . (int)$record->id . '_' . clean_filename($file->get_filename()) : null,
    ...
    public static function import_configuration(string $json, bool $replaceall = false): array {
        ...
        if (!empty($data['sections']) && is_array($data['sections'])) {
            ...
            if (!empty($data['sections'][self::EXPORT_SECTION_SLIDESHOW])) {
                $summary += self::import_easyedu_theme_configuration($data['sections'][self::EXPORT_SECTION_SLIDESHOW]);
            }
            if (!empty($data['sections'][self::EXPORT_SECTION_SITE_BANNERS])) {
                $summary += self::import_easyedu_theme_configuration($data['sections'][self::EXPORT_SECTION_SITE_BANNERS]);
            }
