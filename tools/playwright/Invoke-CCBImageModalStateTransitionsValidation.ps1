...
$playwrightSpec = Join-Path $scriptDir 'ccb-image-modal-state-transitions.spec.js'
...
$runId = 'ccb-image-modal-state-transitions-{0}-{1}' -f [DateTime]::UtcNow.ToString('yyyyMMddTHHmmssfffZ'), $PID
$runRoot = Join-Path $artifactBase ('ccb\image-modal-state-transitions\supervised\' + $runId)
...
function Start-Node([string[]]$Arguments, [string]$WorkingDirectory) {
    ...
    if (-not $process.Start()) { throw 'Unable to start IMG-07 Playwright process.' }
...
function Invoke-Fixture([string]$Command, [string]$Manifest = '') {
    ...
    if (-not $json) { throw "IMG-07 fixture returned no JSON for $Command." }
    ...
    do {
        try {
            return Acquire-EasyEduResourceLease -Resource 'moodle51-active-fixture-write' `
                ...
                -Purpose 'CCB IMG-07 image-modal state-transition validation' `
...
Write-Phase 'preflight' 'started' 'IMG-07 supervisor initialised.'
...
try {
    foreach ($required in @(
        ...
        if (-not (Test-Path -LiteralPath $required)) { throw "Required IMG-07 file is missing: $required" }
    }
    ...
    $imageFixtureA = Join-Path $profile.moodleRoot 'mod\workshop\tests\fixtures\moodlelogo.png'
    $imageFixtureB = Join-Path $profile.moodleRoot 'pix\moodlelogo_grayhat.png'
    foreach ($required in @($playwrightCli, $runtimeConfig, $profile.phpExecutable, $imageFixtureA, $imageFixtureB)) {
        if (-not (Test-Path -LiteralPath $required)) { throw "Required IMG-07 runtime validation file is missing: $required" }
    }
    $tempConfig = Join-Path $runRoot 'playwright.img07.config.js'
    $configSource = @"
...
"@
    ...
    Write-Phase 'playwright-discovery' 'started' 'Selecting exactly one IMG-07 test before credentials or fixture work.'
    $discoveryArgs = @($playwrightCli, 'test', (Split-Path -Leaf $playwrightSpec), ('--config=' + $tempConfig), '--list')
    ...
    if ($LASTEXITCODE -ne 0 -or $discoveryOutput -notmatch 'Total:\s+1\s+test\s+in\s+1\s+file') {
        throw 'Playwright discovery did not select exactly one IMG-07 test.'
    }
    Write-Phase 'playwright-discovery' 'complete' 'Exactly one IMG-07 test selected.'
    ...
    Write-Phase 'lease-acquire' 'complete' 'CCB IMG-07 fixture lease acquired.'
    $fixture = Invoke-Fixture 'setup'
    $manifest = [ordered]@{runId = $runId; categoryid = [int]$fixture.categoryid; sourcekey = [string]$fixture.sourcekey}
    Set-Content -LiteralPath $manifestFile -Value ($manifest | ConvertTo-Json -Depth 10) -Encoding UTF8
    Write-Phase 'fixture-setup' 'complete' "Disposable IMG-07 category $($fixture.categoryid) created."

    . $credentialLoader | Out-Null
    $loadedEnvironment += @('EASYEDU_MOODLE_URL', 'EASYEDU_MOODLE_USERNAME', 'EASYEDU_MOODLE_PASSWORD',
        'CCB_MOODLE_URL', 'CCB_MOODLE_USERNAME', 'CCB_MOODLE_PASSWORD')
    $env:EASYEDU_CCB_IMG07_SOURCE_CATEGORY_ID = [string]$fixture.categoryid
    $env:EASYEDU_CCB_IMG07_ARTIFACT_ROOT = $runRoot
    $env:EASYEDU_CCB_IMG07_MANIFEST = $manifestFile
    $env:EASYEDU_CCB_IMG07_IMAGE_A = $imageFixtureA
    $env:EASYEDU_CCB_IMG07_IMAGE_B = $imageFixtureB
    $loadedEnvironment += @('EASYEDU_CCB_IMG07_SOURCE_CATEGORY_ID', 'EASYEDU_CCB_IMG07_ARTIFACT_ROOT',
        'EASYEDU_CCB_IMG07_MANIFEST', 'EASYEDU_CCB_IMG07_IMAGE_A', 'EASYEDU_CCB_IMG07_IMAGE_B')
    $runArgs = @($playwrightCli, 'test', (Split-Path -Leaf $playwrightSpec), ('--config=' + $tempConfig),
    ...
    Write-Phase 'playwright-child' 'complete' "IMG-07 exit code $childExitCode."
    $child = $null
}
catch {
    ...
    $cleanup = [ordered]@{
        complete = ($null -eq $cleanupError -and (($null -eq $fixture) -or ($cleanupResult -and $cleanupResult.categoryRemoved)))
    ...
    Set-Content -LiteralPath $resultFile -Value (([ordered]@{
        runId = $runId; status = $status; childExitCode = $childExitCode; artifactDirectory = $runRoot
        cleanup = $cleanup; environmentCleared = $true
    ...
    if (Test-Path -LiteralPath $artifactManifestScript) {
        ...
        try {
            & $artifactManifestScript -RunRoot $runRoot -ApprovedRoot $artifactBase `
                ...
                -RetainFile @('img-07-a-crop-applied.png', 'img-07-a-fill-retains-crop.png', 'img-07-b-selected.png',
                    'img-07-a-restored.png', 'img-07-a-recrop-cancel.png', 'img-07-a-redo.png', 'img-07-trace.zip') | Out-Null
        } catch {
