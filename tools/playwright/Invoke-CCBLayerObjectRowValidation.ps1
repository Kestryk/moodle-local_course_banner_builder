...
$playwrightSpec = Join-Path $scriptDir 'ccb-layer-object-row.spec.js'
...
$runId = 'ccb-layer-object-row-{0}-{1}' -f [DateTime]::UtcNow.ToString('yyyyMMddTHHmmssfffZ'), $PID
$runRoot = Join-Path $artifactBase ('ccb\layer-object-row\supervised\' + $runId)
$profile = Join-Path $runRoot 'profile'
$ownedProfiles = @(
    $profile,
    (Join-Path $runRoot 'layer-object-row\100\profile'),
    (Join-Path $runRoot 'layer-object-row\200\profile')
)
...
function Invoke-Fixture([string]$Command, [string]$Argument = '') {
    ...
    if (-not $json) { throw "Layer object-row fixture returned no JSON for $Command." }
    ...
    do {
        try {
            return Acquire-EasyEduResourceLease -Resource 'moodle51-active-fixture-write' `
                ...
                -Purpose 'CCB layer object-row visual validation fixture' -LeaseSeconds ([Math]::Max(180, $WatchdogSeconds + 120))
        } catch {
            $message = $_.Exception.Message
            if ($WaitForLeaseSeconds -le 0 -or $message -notmatch 'already leased' -or (Get-Date) -ge $deadline) { throw }
...
Write-Phase 'preflight' 'started' 'CCB layer object-row validation supervisor initialised.'
...
try {
    ...
    if ([IO.Path]::GetFullPath($runRoot).StartsWith([IO.Path]::GetFullPath($pluginRoot) + [IO.Path]::DirectorySeparatorChar, [StringComparison]::OrdinalIgnoreCase)) {
        throw 'Layer object-row artifact root resolves inside the CCB repository.'
    }
    ...
    $env:EASYEDU_CCB_LAYER_ROW_PROFILE = $profile; $loadedEnvironment += 'EASYEDU_CCB_LAYER_ROW_PROFILE'
    $env:EASYEDU_CCB_LAYER_ROW_ARTIFACT_ROOT = $runRoot; $loadedEnvironment += 'EASYEDU_CCB_LAYER_ROW_ARTIFACT_ROOT'
    $env:EASYEDU_CCB_LAYER_ROW_SOURCE_CATEGORY_ID = '0'; $loadedEnvironment += 'EASYEDU_CCB_LAYER_ROW_SOURCE_CATEGORY_ID'

    Write-Phase 'playwright-discovery' 'started' 'Selecting exactly one layer object-row test before fixture work.'
    ...
    if ($LASTEXITCODE -ne 0 -or $discoveryOutput -notmatch 'Total:\s+1\s+test\s+in\s+1\s+file') {
        throw 'Playwright discovery did not select exactly one CCB layer object-row test.'
    }
    Write-Phase 'playwright-discovery' 'complete' 'Exactly one layer object-row test selected.'
    ...
    Write-Phase 'fixture-setup' 'started' 'Creating disposable layer object-row source.'
    $setup = Invoke-Fixture 'setup'
    $manifest = [ordered]@{
        runId = $runId; categoryid = [int]$setup.categoryid; sourcekey = [string]$setup.sourcekey
        reorderableIds = @($setup.reorderableIds); lockedId = [int]$setup.lockedId
    }
    Set-Content -LiteralPath $manifestFile -Value ($manifest | ConvertTo-Json -Depth 20) -Encoding UTF8
    $env:EASYEDU_CCB_LAYER_ROW_SOURCE_CATEGORY_ID = [string]$setup.categoryid
    Write-Phase 'fixture-setup' 'complete' "Disposable category $($setup.categoryid) contains two reorderable and one locked layer."

    Write-Phase 'playwright-child' 'started' 'Running the one selected layer object-row scenario.'
    ...
    Set-Content -LiteralPath (Join-Path $runRoot 'playwright.stdout.txt') -Value (Safe $child.StandardOutput.ReadToEnd()) -Encoding UTF8
    Set-Content -LiteralPath (Join-Path $runRoot 'playwright.stderr.txt') -Value (Safe $child.StandardError.ReadToEnd()) -Encoding UTF8
    Write-Phase 'playwright-child' 'complete' "Layer object-row scenario exit code $childExitCode."
    $child = $null
}
catch {
    ...
    foreach ($ownedProfile in $ownedProfiles) {
        try {
            $profilesRemoved = (Remove-OwnedProfile $ownedProfile) -and $profilesRemoved
        } catch {
