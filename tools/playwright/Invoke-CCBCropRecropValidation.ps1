...
$playwrightSpec = Join-Path $scriptDir 'ccb-crop-recrop.spec.js'
# This fixture has category ownership, draft-file cleanup and zero-elements verification.
...
$runId = 'ccb-crop-recrop-{0}-{1}' -f [DateTime]::UtcNow.ToString('yyyyMMddTHHmmssfffZ'), $PID
$runRoot = Join-Path $artifactBase ('ccb\crop-recrop\supervised\' + $runId)
...
function Start-Node([string[]]$Arguments, [string]$WorkingDirectory) {
    ...
    $info.FileName = 'node.exe'; $info.WorkingDirectory = $WorkingDirectory; $info.UseShellExecute = $false; $info.CreateNoWindow = $true
    ...
    if (-not $process.Start()) { throw 'Unable to start Crop/Recrop Playwright process.' }
...
function Stop-Child {
    if ($child -and -not $child.HasExited) { try { & taskkill.exe /PID $child.Id /T /F 2>$null | Out-Null } catch { } }
}
...
function Invoke-Fixture([string]$Command, [string]$Manifest = '') {
    ...
    if (-not $json) { throw "Crop/Recrop fixture returned no JSON for $Command." }
...
function Acquire-ValidationLease {
    ...
    do {
        try {
            return Acquire-EasyEduResourceLease -Resource 'moodle51-active-fixture-write' -ProjectNamespace 'ccb' `
                -RunId $runId -Repository $pluginRoot -Purpose 'CCB Crop/Recrop QA validation' `
...
Write-Phase 'preflight' 'started' 'EED-CCB-2026-0043-QA1 supervisor initialised.'
...
try {
    foreach ($required in @($orchestrationModule, $statusScript, $artifactManifestScript, $profileFile, $credentialLoader, $playwrightSpec, $fixtureHelper)) {
        if (-not (Test-Path -LiteralPath $required)) { throw "Required Crop/Recrop file is missing: $required" }
    }
    ...
    $tempConfig = Join-Path $runRoot 'playwright.crop-recrop.config.js'
    $configSource = @"
...
"@
    ...
    Write-Phase 'playwright-discovery' 'started' 'Selecting exactly one Crop/Recrop test before credentials or fixture work.'
    ...
    if ($LASTEXITCODE -ne 0 -or $discoveryOutput -notmatch 'Total:\s+1\s+test\s+in\s+1\s+file') { throw 'Playwright discovery did not select exactly one Crop/Recrop test.' }
    Write-Phase 'playwright-discovery' 'complete' 'Exactly one Crop/Recrop test selected.'
    ...
    $lease = Acquire-ValidationLease; Write-Phase 'lease-acquire' 'complete' 'CCB Crop/Recrop fixture lease acquired.'
    $fixture = Invoke-Fixture 'setup'
    Set-Content -LiteralPath $manifestFile -Value ([ordered]@{runId = $runId; categoryid = [int]$fixture.categoryid; sourcekey = [string]$fixture.sourcekey} | ConvertTo-Json) -Encoding UTF8
    Write-Phase 'fixture-setup' 'complete' "Disposable Crop/Recrop category $($fixture.categoryid) created."
    ...
    $env:EASYEDU_CCB_CROP_SOURCE_CATEGORY_ID = [string]$fixture.categoryid
    $env:EASYEDU_CCB_CROP_ARTIFACT_ROOT = $runRoot
    $env:EASYEDU_CCB_CROP_MANIFEST = $manifestFile
    $env:EASYEDU_CCB_CROP_IMAGE_FIXTURE = $imageFixture
    $loadedEnvironment += @('EASYEDU_CCB_CROP_SOURCE_CATEGORY_ID', 'EASYEDU_CCB_CROP_ARTIFACT_ROOT', 'EASYEDU_CCB_CROP_MANIFEST', 'EASYEDU_CCB_CROP_IMAGE_FIXTURE')
    ...
    Write-Phase 'playwright-child' 'complete' "Crop/Recrop exit code $childExitCode."
    $child = $null
} catch {
    ...
    if (Test-Path -LiteralPath $artifactManifestScript) {
        ...
        try { & $artifactManifestScript -RunRoot $runRoot -ApprovedRoot $artifactBase -ProjectNamespace ccb -RunId $runId -Status $retentionStatus -RetainFile @('crop-recrop-1440-before.png','crop-recrop-1440-after-initial.png','crop-recrop-1440-after-cancel.png','crop-recrop-760-before.png','crop-recrop-760-after-switch.png','ccb-crop-recrop-evidence.json') | Out-Null } catch { Write-Phase 'artifact-manifest' 'error' $_.Exception.Message }
    }
}
