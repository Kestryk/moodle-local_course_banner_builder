...
$runId = 'ccb-async-editor-actions-{0}-{1}' -f [DateTime]::UtcNow.ToString('yyyyMMddTHHmmssfffZ'), $PID
$runRoot = Join-Path $artifactBase ('ccb\async-editor-actions\supervised\' + $runId)
...
function Invoke-Fixture([string]$Command, [string]$Argument = '') {
    ...
    if (!$json) { throw "Async editor fixture returned no JSON for $Command." }
...
function Start-Node([string[]]$Arguments) {
    ...
    if (!$process.Start()) { throw 'Unable to start Playwright Node process.' }
    return $process
}
...
try {
    ...
    if ([IO.Path]::GetFullPath($runRoot).StartsWith([IO.Path]::GetFullPath($pluginRoot) + [IO.Path]::DirectorySeparatorChar, [StringComparison]::OrdinalIgnoreCase)) {
        throw 'Async editor artifact root resolves inside the CCB repository.'
    }
    ...
    if ($LASTEXITCODE -ne 0 -or $discovery -notmatch 'Total:\s+1\s+test\s+in\s+1\s+file') {
        throw 'Playwright discovery did not select exactly one Async editor scenario.'
    }
    ...
    $leaseParameters = @{
        ...
        Purpose = 'CCB Async editor destructive-action validation fixture'
    ...
    $env:EASYEDU_CCB_ASYNC_EDITOR_CATEGORY_ID = [string]$setup.categoryid
    ...
    $cleanupResult = $null
    try { if ($manifest -and (Test-Path -LiteralPath $manifestFile)) { $cleanupResult = Invoke-Fixture 'cleanup' $manifestFile } } catch { $cleanupError = Safe $_.Exception.Message }
