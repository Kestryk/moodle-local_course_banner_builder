...
test('CCB primary accordions keep desktop parity with Course Banner options', async({page}) => {
    const env = environment();
    await login(page, env);
    await page.goto(
        `${env.baseUrl}/local/course_banner_builder/admin_manage.php?sourcecategoryid=${encodeURIComponent(env.categoryId)}`,
    ...
    await reference.evaluate(details => { details.open = true; });
    ...
    for (const surface of [selectedSurface, configuredSurface]) {
        ...
        expect(surface.paddingTop).toBe(referenceSurface.paddingTop);
        expect(surface.paddingRight).toBe(referenceSurface.paddingRight);
        expect(surface.paddingBottom).toBe(referenceSurface.paddingBottom);
        expect(surface.paddingLeft).toBe(referenceSurface.paddingLeft);
