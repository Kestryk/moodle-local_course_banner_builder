...
| IMG-06 | Add one image, resize, then use Fit to preview | The new draft starts at original geometry. Side controls are available for the selected image and, on first interaction, convert its visible box to the existing custom placement state. Pointer resize uses that same state. Fit switches it to proportional preview geometry, centres it, resets offsets and persists that state after save/reopen. Cancel leaves the saved layer unchanged. Crop/recrop, drag and modal undo/redo are checked as non-regressions only. `tools/test-image-modal-transform-contract.ps1` protects the source/build contract before the leased visual flow. | One leased Moodle 5.1 Playwright scenario plus manual review | Candidate |
