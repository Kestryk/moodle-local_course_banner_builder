...
| IMG-06 | Add one image, resize, then use Fit to preview | The new draft starts at original geometry. The active draft's technical visual twin is excluded from snap/guide peers so dragging remains direct. Aspect-locked vertical edges and corners follow the moved pointer axis. Live pointer frames synchronize linked sliders once, defer handle/popover measurement and cap peer snapping on complex previews. Side controls and pointer resize update custom placement without generic duplicate serialization. Fit commits proportional preview geometry, centres it, resets offsets and renders the active draft once; the state persists after save/reopen. Cancel leaves the saved layer unchanged. Crop/recrop and modal undo/redo are checked as non-regressions only. `tools/test-image-modal-transform-contract.ps1` protects these contracts before the leased visual flow. | One leased Moodle 5.1 Playwright scenario plus manual review | Candidate |
