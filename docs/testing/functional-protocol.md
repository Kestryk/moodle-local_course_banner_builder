...
| LOAD-03 | Slideshow identity and Skeleton handoff | The localised Course Banner Builder name, title and description appear before the unchanged Navigation in both loading and ready states. The two Skeleton cards have static primary top borders and animated cues only; Course/Site icons are equally centred. At 390 px cards stack, RTL reverses cues only, reduced-motion stops shimmer, forced-colors retains visible borders, and no Skeleton control is focusable. | Playwright/manual | Candidate |
