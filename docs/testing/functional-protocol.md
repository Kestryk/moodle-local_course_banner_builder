...
| IMG-06 | Add one image, resize, then use Fit to preview | The new draft starts at original geometry. Fit, Fill banner, active-draft selection, drag and resize pass through one modal transaction: form fields, active draft JSON, then current/visual DOM mirrors. The transaction resolves the modal-scoped `fitmodeoverride` by id and then canonical name, so id-less Moodle hidden inputs are usable. The active draft's technical visual twin is excluded from snap/guide peers so dragging remains direct. Aspect-locked vertical edges and corners follow the moved pointer axis. A live pointer frame renders at most once and stages JSON until release; filemanager mutations share one form-local refresh queue. Edit-image and general-preview transforms are outside this flow. The state persists after save/reopen; Cancel leaves the saved layer unchanged. Crop/recrop and modal undo/redo are checked as non-regressions only. `tools/test-image-modal-transform-contract.ps1` protects these contracts. `tools/playwright/Invoke-CCBImageModalTransformValidation.ps1` then selects exactly one `ccb-image-modal-transform.spec.js` test before credentials or fixture setup, holds the shared Moodle fixture lease, stores geometry/timing/captures/trace outside Git and verifies cleanup. | One local-supervised leased Moodle 5.1 Playwright scenario plus manual review | Candidate |
| IMG-07 | Image-modal state transitions with two draft images | Starting from the hardened IMG-06 upload path, upload A and B. The single supervised scenario applies a Crop on A, applies Fill without clearing A's four Crop values, switches A → B → A and proves the current layer, visual twin and draft JSON retain those values. It then recrops A and Cancels without losing the applied Crop, applies a second recrop, and proves Undo then Redo restore the two expected Crop states. It records per-gesture durations and PerformanceObserver long tasks, retains final transition captures and trace outside Git, and always cleans the disposable fixture and lease. `tools/playwright/Invoke-CCBImageModalStateTransitionsValidation.ps1` discovers exactly one `ccb-image-modal-state-transitions.spec.js` test before credentials or fixture setup, with one worker and zero retries. It does not change Crop implementation, AMD assets or the add-image product flow. | One local-supervised leased Moodle 5.1 Playwright scenario plus manual review | Candidate |
