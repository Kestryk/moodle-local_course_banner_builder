...
| IMG-06 | Add one image, resize, then use Fit to preview | The new draft starts at original geometry. Form-rendered corner/edge handles and the dynamically rendered Fit control keep direct modal event bindings. Side controls and pointer resize first commit the visible box to the draft state, then update it as custom placement. Fit commits proportional preview geometry, centres it, resets offsets and persists that state after save/reopen. Cancel leaves the saved layer unchanged. Crop/recrop, drag and modal undo/redo are checked as non-regressions only. `tools/test-image-modal-transform-contract.ps1` protects the local-event and commit-before-rerender contracts before the leased visual flow. | One leased Moodle 5.1 Playwright scenario plus manual review | Candidate |
