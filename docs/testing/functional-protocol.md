...
| IMG-06 | Add one image, resize, then use Fit to preview | The new draft starts at original geometry. The active draft's technical visual twin is excluded from snap/guide peers so dragging remains direct. Aspect-locked vertical edges and corners follow the moved pointer axis. Live pointer frames synchronize linked sliders once, defer handle/popover measurement and cap peer snapping on complex previews. In the add-image form (`elementid = 0`), pointer handles start the dedicated draft resize path and Fit writes proportional centred geometry directly to the active draft before rendering it. Both paths resolve Moodle's hidden `fitmodeoverride` by scoped name when the input has no generated id. Edit-image and general-preview transforms are outside this flow. The state persists after save/reopen; Cancel leaves the saved layer unchanged. Crop/recrop and modal undo/redo are checked as non-regressions only. `tools/test-image-modal-transform-contract.ps1` protects these contracts. `tools/playwright/Invoke-CCBImageModalTransformValidation.ps1` then selects exactly one `ccb-image-modal-transform.spec.js` test before credentials or fixture setup, holds the shared Moodle fixture lease, stores geometry/timing/captures/trace outside Git and verifies cleanup. | One local-supervised leased Moodle 5.1 Playwright scenario plus manual review | Candidate |
...
| LYR-04 | Inspect layer identity, Sort order status and Layer infos disclosure | Image, Border and Overlay types stay in the coloured leading rail. Sort order shows only one copy of each status badge and its lock badge below the localised `Locked` label; neither status nor lock appears in Banner layer. A default image with no override or meaningful layer information has neither `Layer infos & overrides` disclosure nor `?` help. A dynamic or otherwise populated layer retains its disclosure, keyboard help and existing content. Every rendered popover indicator remains keyboard-focusable. Native drag and selection remain available. | One leased Moodle 5.1 Playwright scenario (`ccb-layer-object-row.spec.js`) | Candidate |
