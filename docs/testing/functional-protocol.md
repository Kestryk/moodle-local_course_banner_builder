...
| LYR-04 | Inspect layer identity, Sort order status and Layer infos disclosure | Image, Border and Overlay types stay in the coloured leading rail. Sort order shows only one copy of each status badge and its lock badge below the localised `Locked` label; neither status nor lock appears in Banner layer. A layer with no display, border or overlay summary has neither `Layer infos & overrides` disclosure nor `?` help. A populated summary retains its disclosure, keyboard help and existing content. Every rendered popover indicator remains keyboard-focusable. Native drag and selection remain available. | One leased Moodle 5.1 Playwright scenario (`ccb-layer-object-row.spec.js`) | Candidate |
