...
| Visual-editor save | JSON payload parsed server-side, sesskey, stored-source prerequisite and manager validation | Redirect; warning when source settings do not exist | `EED-CCB-2026-0020` owns the narrow JSON local-refresh route and its stale-payload lock; see `general-preview-async-actions.md`. |
