...
The official Sass entry point rebuilds all of `styles.css`. At this base it also
emits pre-existing Guide declarations that were already imported by the source
entry point but missing from the committed generated asset. No Guide source,
template, AMD module or behavior belongs to this batch; the generated output is
kept because it is the reproducible result of the required official build.
...
overflow of the identity itself and no state-changing requests. It uses process-local credentials and
external artifacts only when separately authorised.

The plugin declares Moodle 4.5 as its compatibility floor. This batch uses
existing Moodle output and Mustache APIs only; static validation does not prove
the deferred Moodle 4.5, 5.1 or 5.2 runtime matrix.
